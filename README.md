...
В папке Demo находится пример приложения, использующего **SecureBridge** (Devart) для подключения к SSH серверам.

**Примечание:** Для компиляции демо-приложения требуются компоненты SecureBridge. Вы можете легко заменить их на Indy или TClientSocket, так как терминал не зависит от сетевой библиотеки.
...
Проект распространяется под лицензией MIT. См. файл [LICENSE](https://www.google.com/search?q=LICENSE) для подробностей.
